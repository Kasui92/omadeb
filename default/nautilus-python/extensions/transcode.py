...
class TranscodeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcode(self, paths):
        wrapper = shutil.which("omadeb-launch-terminal-with-presentation")
        binary = shutil.which("omadeb-transcode")
    ...
    def _make_item(self, paths):
        ...
        item = Nautilus.MenuItem(
            name="OmadebTranscodeNautilus::transcode",
            label=label,
    ...
    def _tools_available(self):
        return bool(
            shutil.which("omadeb-launch-terminal-with-presentation")
            and shutil.which("omadeb-transcode")
        )
